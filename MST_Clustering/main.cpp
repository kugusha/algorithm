...
Point2D draw_random(const vector<Point2D>& vec) {
    size_t ind = size_t(rand() % vec.size());
    return vec[ind];
}
...
vector<size_t> ClusterGraph(vector<Edge> edges,  // copy for sorting
                            size_t vertexCount,
                            size_t clusterCount)  {
...
void PrintClusters2D(const vector<Point2D>& points,
...
int main() {
    auto points = Random2DClusters(3, 1000);
    ...
    PrintClusters2D(points, labels, clustersCount, "data_new");
